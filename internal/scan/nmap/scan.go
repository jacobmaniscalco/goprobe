...
import (
	...
	"github.com/jacobmaniscalco/goprobe/ui/styles"
	"github.com/jacobmaniscalco/goprobe/internal/scan"
)

func PerformScan(options scan.ScanOptions) (string, error) {

	nmapOptions := []nmap.Option{
		nmap.WithTargets(options.Host),
	}
...
func formatScanResults(results *nmap.Run, warnings *[]string) string {
	...
	for _, host := range results.Hosts {
		...
		t := table.New().
			...
			Headers("Port", "State", "Service", "Product").
			Rows(rows...)
